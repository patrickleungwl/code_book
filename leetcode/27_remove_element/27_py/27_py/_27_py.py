...
# of nums being 2.
#It doesn't matter what you leave beyond the returned length. For example if you 
#return 2 with nums = [2,2,3,3] or nums = [2,3,0,0], your answer will be accepted.

# Ex 2
#Input: nums = [0,1,2,2,3,0,4,2], val = 2
#Output: 5, nums = [0,1,4,0,3]
#Explanation: Your function should return length = 5, with the first five elements 
#of nums containing 0, 1, 3, 0, and 4. Note that the order of those five elements can 
...
class Solution:
    def removeElement(self, nums: List[int], val: int) -> int:
        check_idx=0     # place where we're checking
        good_idx=0      # place where we're putting in good items 
        
        for idx in range(0,len(nums)):
             if nums[idx]!=val:
                 nums[good_idx]=nums[idx]
                 good_idx+=1
        return good_idx
